...
<div x-cloak>
    <div x-show="editModal"  class="fixed top-0 bottom-0 left-0 right-0 z-50 flex items-center justify-center overflow-auto bg-gray-700 bg-opacity-70" x-transition:enter="transition ease duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease duration-300" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
        <div x-show="editModal" @click.away="editModal = !editModal" class="w-1/2 px-10 py-8 mx-4 bg-white rounded shadow-lg h-max" x-transition:enter="transition ease duration-100 transform" x-transition:enter-start="opacity-0 scale-90 translate-y-1" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease duration-100 transform" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-90 translate-y-1">
            ...
            <div class="flex justify-end mt-4">
                <button @click="editModal = !editModal;" class="mr-6 text-lg font-semibold text-gray-300 poppins">Anulează</button>
                <button wire:click='saveBook'  class="text-lg font-semibold text-green-700 poppins">Salvează</button>
            </div>
        </div>
    </div>
</div>
